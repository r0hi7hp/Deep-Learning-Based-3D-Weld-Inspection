"""
Hybrid Model Weld Detection Predictor - Production Ready (Data Augmentation Version)
...
class HybridPredictor:
    """Production-ready Hybrid Model predictor for weld detection."""
    
    def __init__(self, model_path: str, num_classes: int = 2, num_points: int = 2048, num_heads: int = 4):
        """
    ...
    def _load_model(self, model_path: str) -> HybridPointNet:
        """Load and prepare the model for inference."""
        model = HybridPointNet(num_classes=self.num_classes, num_heads=self.num_heads).to(self.device)
...
def predict(ply_path: str, model_path: str, num_points: int = 2048) -> Dict:
    ...
    """
    predictor = HybridPredictor(model_path, num_points=num_points)
    try:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Hybrid Model Weld Detection')
    parser.add_argument('ply_path', type=str, help='Path to input PLY file')
    parser.add_argument('--model', type=str, default='checkpoints/Training 1/best_model.pth', 
